#!/usr/bin/env python3
"""
Comprehensive Security Audit for askmoss.com
...
load_dotenv('/home/robert/Work/_archive/email-forensics/.env')

SERVICE_ACCOUNT_EMAIL = os.getenv('SERVICE_ACCOUNT_EMAIL')
ADMIN_USER = os.getenv('ADMIN_USER')

# Attack window for this tenant
ATTACK_START = datetime(2025, 12, 1, 0, 0, 0)
ATTACK_END = datetime(2025, 12, 18, 0, 0, 0)

# Known attacker IPs for this tenant (Lori Maynard compromise)
ATTACKER_IPS = {
    '172.120.137.37',
    '45.87.125.150',
    '46.232.34.229',
...
def check_admin_changes():
    ...
    print("\n" + "=" * 80)
    print("ADMIN CHANGES DURING COMPROMISE WINDOW (Dec 1-18)")
    print("=" * 80)
    ...
    if security_changes:
        for evt in security_changes[:20]:
...
def check_oauth_tokens():
    ...
    print("\n--- SUSPICIOUS APP GRANTS ---")
    if suspicious_grants:
        for grant in suspicious_grants[:20]:
...
def check_mobile_devices():
    ...
    compromised_user = 'lori.maynard@askmoss.com'
...
def main():
    print("=" * 80)
    print("COMPREHENSIVE SECURITY AUDIT - askmoss.com")
    ...
    # 4. Check all users Gmail settings
    print("\n" + "=" * 80)
    print("ALL-USER GMAIL SETTINGS AUDIT")
    print("=" * 80)
            ...
            for issue_type, issues in by_type.items():
                ...
                for issue in issues:
                    if issue_type == 'FILTER':
                        ...
                        print(f"    ⚠️ {issue['user']}: forwarding address {issue['address']}")
                    elif issue_type == 'DELEGATE':
                        print(f"    ⚠️ {issue['user']}: delegate {issue['delegate']}")
        else:
    ...
    if issues_found == 0 and failed == 0:
        ...
        if failed > 0:
            print(f"\n  ⚠️ Gmail settings incomplete - add gmail.settings.basic scope")
